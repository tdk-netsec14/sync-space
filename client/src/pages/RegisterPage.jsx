        ...
        <div className="relative z-10">
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="font-editorial text-lg font-bold tracking-tight text-white">
              Sync<span className="font-serif-editorial italic font-normal text-brand-lavender lowercase ml-0.5">space</span>
            </span>
          </Link>
          ...
          <h1 className="mt-28 max-w-md font-editorial text-5xl font-extrabold tracking-tight leading-none text-white uppercase">
            Create <br />
            your team <br />
            <span className="font-serif-editorial italic font-normal text-brand-yellow lowercase">grid today</span>.
          </h1>
          <p className="mt-8 max-w-sm text-brand-offwhite/60 leading-relaxed text-sm font-sans-editorial">
            Join the new standard of sprint canvases. Fluid columns, multi-tenant boundaries, and instant socketed boards.
          </p>
        </div>
      ...
      <main className="flex items-center justify-center px-6 py-12 relative dot-grid">
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <h2 className="font-editorial text-3xl font-bold tracking-tight text-brand-black uppercase">Create account</h2>
              <p className="mt-2 text-xs font-sans-editorial font-bold text-brand-black/45">Configure your custom agile command console.</p>
              ...
              {inviteToken ? (
                <div className="mt-4 flex items-start gap-3 rounded-2xl border-editorial border-brand-purple bg-brand-lavender/10 p-4 text-xs font-bold text-brand-black">
                  ...
                  <div>
                    <span className="font-editorial uppercase text-brand-purple tracking-wider text-[10px]">Workspace Invitation</span>
            ...
            <div className="space-y-3.5">
              <label className="block">
                ...
                />
                {errors.name ? <p className="mt-1 text-[11px] font-bold text-red-500">{errors.name}</p> : null}
              </label>
              ...
              <label className="block">
                ...
                <input 
                  ...
                  className="w-full rounded-2xl border border-brand-black bg-brand-offwhite px-4 py-3 text-sm text-brand-black outline-none transition-all placeholder:text-brand-black/30 font-medium focus:bg-white focus:shadow-editorial-sm" 
                />
                {errors.email ? <p className="mt-1 text-[11px] font-bold text-red-500">{errors.email}</p> : null}
              </label>

              <label className="block">
                <span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Password</span>
                <input 
                  type="password" 
                  name="password" 
                  value={form.password} 
                  ...
                  className="w-full rounded-2xl border border-brand-black bg-brand-offwhite px-4 py-3 text-sm text-brand-black outline-none transition-all placeholder:text-brand-black/30 font-medium focus:bg-white focus:shadow-editorial-sm" 
                />
                {errors.password ? <p className="mt-1 text-[11px] font-bold text-red-500">{errors.password}</p> : null}
              </label>
            </div>
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              ...
              >
                {isSubmitting ? <LoaderCircle className="h-4.5 w-4.5 animate-spin text-brand-yellow" /> : null}
            ...
            <p className="mt-6 text-center text-xs text-brand-black/55 font-sans-editorial font-bold">
              ...
              <Link to={inviteToken ? `/login?invite=${inviteToken}` : "/login"} className="text-brand-purple hover:underline">
                Sign in instead
              </Link>
            </p>
          </form>
