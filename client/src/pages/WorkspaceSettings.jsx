...
import { Menu, ShieldAlert, Sparkles, Users, Trash2, LogOut, Settings, Terminal, BarChart2, Briefcase, Timer, Palette, Smile } from 'lucide-react';
import Sidebar from '../components/Sidebar';
import { deleteWorkspace, fetchMembers, getWorkspace, removeMember, updateMemberRole, updateWorkspace, leaveWorkspace } from '../services/api';
...
export default function WorkspaceSettings() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [workspaceResponse, membersResponse] = await Promise.all([getWorkspace(workspaceId), fetchMembers(workspaceId)]);
        if (!active) return;
  ...
  async function handleRoleChange(userId, role) {
    ...
    setMembers((current) => current.map((member) => (member.user?.id === userId ? { ...member, role: response.data.member.role } : member)));
  }
    ...
    <div className="min-h-screen bg-brand-offwhite text-brand-black relative dot-grid flex overflow-x-hidden">
      ...
      <main className="flex-1 p-6 md:ml-[260px] md:p-10 max-w-5xl mx-auto w-full transition-all">
        ...
        <div className="mb-8 flex items-start gap-4">
          ...
          <div>
            <span className="text-[9px] font-black uppercase tracking-widest text-brand-black/45 block font-sans-editorial">Control Console</span>
            <h1 className="font-editorial text-3xl sm:text-4xl font-black text-brand-black leading-none uppercase mt-1">Workspace settings</h1>
        ...
        {/* Tab content panels */}
        {tab === 'general' && (
          <form onSubmit={handleSave} className="max-w-2xl border-editorial bg-white p-6 sm:p-8 shadow-editorial rounded-3xl relative overflow-hidden flex flex-col">
            ...
            <div className="space-y-4 mb-6">
              <label className="block">
                <span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Workspace Name</span>
                <input 
                  name="name" 
                  value={form.name} 
              ...
              <label className="block">
                <span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Description</span>
                <textarea 
                  name="description" 
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-6 gap-2.5">
                  {['#8B5CF6', '#0ea5e9', '#10b981', '#f59e0b', '#ef4444', '#DCC7FF'].map((color) => (
                    <button 
                      key={color} 
                      type="button" 
              ...
              {members.map((member) => (
                ...
                >
                  <div className="min-w-0">
                    <p className="font-editorial text-xs font-bold uppercase tracking-wider text-brand-black truncate">{member.user?.name}</p>
                    <p className="text-[11px] text-brand-black/45 font-sans-editorial font-bold truncate mt-0.5">{member.user?.email}</p>
                  </div>
        ...
        {tab === 'danger' && (
          <section className="max-w-2xl border-editorial bg-white p-6 sm:p-8 shadow-editorial rounded-3xl relative overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-2 text-rose-700 border-b border-brand-black/10 pb-3 mb-5">
              ...
              <h2 className="font-editorial text-base font-bold uppercase tracking-wider">Danger Zone</h2>
            </div>
            
            <p className="text-xs font-sans-editorial font-bold text-brand-black/50 leading-relaxed">
              These operations are destructive. Revoking permissions or deleting the active workspace will wipe all Sprint Tracks, boards, tasks, and history permanently.
            </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            >
              ...
              <div className="flex items-center gap-2 text-rose-600 mb-4">
                <ShieldAlert className="h-5 w-5 animate-bounce shrink-0" />
                <h3 className="font-editorial text-sm font-bold uppercase tracking-widest">Confirm deletion</h3>
              </div>
              ...
              <p className="text-xs font-sans-editorial font-bold text-brand-black/50 leading-relaxed mb-6">
                This process is completely irreversible. All sprint boards, task cards, teammates, history logs, and details associated with <span className="text-rose-600 font-extrabold">{workspace?.name}</span> will be deleted permanently.
              </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            >
              ...
              <div className="flex items-center gap-2 text-brand-yellow mb-4">
                ...
                <h3 className="font-editorial text-sm font-bold uppercase tracking-widest text-brand-black">Confirm exit</h3>
              </div>
              <p className="font-editorial text-lg font-black text-brand-black uppercase leading-tight mb-3">
                Are you sure you want to leave this workspace?
              </p>
              <p className="text-xs font-sans-editorial font-bold text-brand-black/50 leading-relaxed mb-6">
                You will lose all operator access, active assignments, and administrative controls within the workspace <span className="text-brand-black font-extrabold">{workspace?.name}</span>. An admin will need to invite you back to restore access.
              </p>
              <div className="flex gap-3 justify-end mt-auto">
                <button
                  type="button"
