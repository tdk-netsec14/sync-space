...
import { Copy, MailPlus, Menu, CheckCircle2, UserCircle2 } from 'lucide-react';
import { useParams } from 'react-router-dom';
import Sidebar from '../components/Sidebar';
import ActivityFeed from '../components/ActivityFeed';
import { useSocket } from '../context/SocketContext';
import { useWorkspace } from '../context/WorkspaceContext';
import { createInvite, fetchMembers, fetchWorkspaceActivity, fetchWorkspaceStats, getWorkspace } from '../services/api';
...
export default function WorkspaceDashboard() {
  ...
  const [stats, setStats] = useState({ totalTasks: 0, completedThisWeek: 0, activeMembers: 0, overdueTasks: 0 });
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [workspaceResponse, membersResponse, statsResponse, activityResponse] = await Promise.all([
        ...
        setStats(statsResponse.data.stats || { totalTasks: 0, completedThisWeek: 0, activeMembers: 0, overdueTasks: 0 });
  ...
  useEffect(() => {
    ...
    async function refreshStats() {
      try {
        ...
        setStats(response.data.stats || { totalTasks: 0, completedThisWeek: 0, activeMembers: 0, overdueTasks: 0 });
      } catch (error) {
    ...
    const onActivity = ({ activity }) => {
      ...
      setActivities((current) => [activity, ...current.filter((item) => String(item.id) !== String(activity.id))].slice(0, 50));
      void refreshStats();
    };
  ...
  async function loadMoreActivities() {
    ...
    try {
      const response = await fetchWorkspaceActivity(workspaceId, { limit: 8, before: activityCursor });
      ...
      <main className="flex-1 p-6 md:ml-[260px] md:p-10 animate-fade-in">
        ...
        <section className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="border-editorial bg-white p-6 shadow-editorial rounded-3xl relative overflow-hidden transition-all duration-300 shadow-editorial-hover">
            <span className="absolute left-0 top-0 bottom-0 w-[6px]" style={{ backgroundColor: themeColor }} />
            <p className="text-[10px] font-bold uppercase tracking-wider text-brand-black/45 font-sans-editorial">Total Cards</p>
          ...
          <div className="border-editorial bg-white p-6 shadow-editorial rounded-3xl relative overflow-hidden transition-all duration-300 shadow-editorial-hover">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-brand-black/45 font-sans-editorial">Completed Tasks</p>
            <div className="mt-4 text-5xl font-editorial font-black text-brand-black tracking-tight leading-none">
          ...
          <div className="border-editorial bg-white p-6 shadow-editorial rounded-3xl relative overflow-hidden transition-all duration-300 shadow-editorial-hover">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-brand-black/45 font-sans-editorial">Active Seats</p>
            <div className="mt-4 flex items-baseline gap-1 text-5xl font-editorial font-black text-brand-black tracking-tight leading-none">
              {stats.activeMembers}
              <span className="text-[9px] font-bold text-brand-black/35 font-sans-editorial uppercase tracking-wider ml-1">occupied</span>
            </div>
          </div>
          ...
          <div className="border-editorial bg-white p-6 shadow-editorial rounded-3xl relative overflow-hidden transition-all duration-300 shadow-editorial-hover">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-brand-black/45 font-sans-editorial">Overdue Items</p>
            <div className="mt-4 text-5xl font-editorial font-black text-brand-black tracking-tight leading-none">
              {stats.overdueTasks}
            </div>
          </div>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.15fr_0.85fr] items-start">
          ...
          <ActivityFeed activities={activities} onLoadMore={hasMoreActivities ? loadMoreActivities : null} loadingMore={loadingMoreActivities} />
          ...
          <section className="border-editorial rounded-3xl bg-white p-6 shadow-editorial relative overflow-hidden">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-brand-black/10 pb-5">
              <div>
                <h2 className="font-editorial text-xl font-bold tracking-tight text-brand-black uppercase">Grid Operators</h2>
                <p className="text-[10px] font-sans-editorial font-bold text-brand-black/45 mt-1">Authorized seats allocated on this space.</p>
              </div>
              {inviteUrl && (
            ...
            {inviteMessage && (
              ...
              >
                ...
                <div>
                  <span className="text-[10px] uppercase font-editorial tracking-wider text-brand-purple">Invitation Generated</span>
                  <p className="mt-1 font-sans-editorial text-brand-black/85 leading-normal font-medium">{inviteMessage}</p>
                </div>
              </motion.div>
            )}
              ...
              {members.map((member) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <div 
                      className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-black text-brand-black shadow-sm border border-brand-black/10"
                      style={{ backgroundColor: themeColor }}
                    >
                      {member.user?.name?.[0]?.toUpperCase() || 'U'}
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-xs font-editorial font-bold text-brand-black leading-none">{member.user?.name}</p>
                      <p className="truncate text-[10px] text-brand-black/45 font-medium font-sans-editorial mt-1.5">{member.user?.email}</p>
                    </div>
                  </div>
