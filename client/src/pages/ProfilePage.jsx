...
import { ArrowLeft, BadgeCheck, ShieldAlert, Sparkles, Terminal, BarChart2, Briefcase, Users, Timer, Palette } from 'lucide-react';
    ...
    <div className="min-h-screen bg-brand-offwhite px-6 py-12 text-brand-black font-sans antialiased dot-grid relative">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-brand-black/10 pb-8 mb-8">
          <div>
            <span className="text-[9px] font-black uppercase tracking-widest text-brand-black/45 block font-sans-editorial">Identity Registry</span>
            <h1 className="font-editorial text-3xl sm:text-4xl font-black text-brand-black leading-none uppercase mt-1">Account Configuration</h1>
            <p className="text-xs font-sans-editorial font-bold text-brand-black/45 mt-2 leading-relaxed">Configure personal sprint credentials and secure key credentials.</p>
          </div>
        ...
        {/* Message Banner */}
        {message && (
          <div className={`mb-6 rounded-2xl border px-4 py-3.5 text-xs font-bold flex items-center gap-2 animate-fade-in font-sans-editorial ${
            isSuccess 
              ? 'border-emerald-500 bg-emerald-50 text-emerald-800' 
              : 'border-rose-500 bg-rose-50 text-rose-800'
          }`}>
            {isSuccess ? <BadgeCheck className="h-4.5 w-4.5 text-emerald-600 shrink-0" /> : <ShieldAlert className="h-4.5 w-4.5 text-rose-600 shrink-0" />}
            <span>{message}</span>
          </div>
        )}
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <form onSubmit={saveProfile} className="border-editorial bg-white p-6 shadow-editorial rounded-3xl flex flex-col relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-brand-yellow" />
            <h2 className="font-editorial text-sm font-bold text-brand-black border-b border-brand-black/10 pb-3 mb-5 uppercase tracking-widest">Identity details</h2>
            
            <div className="space-y-4 flex-1">
              <label className="block">
                <span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Full Name</span>
                <input 
                  value={profile.name} 
              ...
              <label className="block">
                <span className="mb-2 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Email Address</span>
                <input 
                  value={user?.email || ''} 
                  readOnly 
              ...
              <div>
                <span className="mb-3 block text-[10px] font-bold uppercase tracking-wider text-brand-black/65 font-sans-editorial">Avatar color tag</span>
          ...
          {/* Security Form */}
          <form onSubmit={savePassword} className="border-editorial bg-white p-6 shadow-editorial rounded-3xl flex flex-col relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-1.5 bg-brand-purple" />
            <h2 className="font-editorial text-sm font-bold text-brand-black border-b border-brand-black/10 pb-3 mb-5 uppercase tracking-widest">Security credential</h2>
        ...
        <section className="mt-8 border-editorial bg-white p-6 shadow-editorial rounded-3xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-brand-black/10 pb-3 mb-5">
            <h2 className="font-editorial text-sm font-bold text-brand-black uppercase tracking-widest">Workspace Memberships</h2>
              ...
              >
                ...
                >
                  {emojiMap[membership.workspace.logo] ? (
                    React.createElement(emojiMap[membership.workspace.logo], { className: "h-5 w-5 text-brand-black" })
                  ) : (
                    <span className="text-sm font-black">{membership.workspace.logo || 'S'}</span>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-xs font-bold text-brand-black group-hover:text-brand-purple transition-all truncate font-editorial uppercase tracking-wider">{membership.workspace.name}</p>
