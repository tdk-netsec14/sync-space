...
import { LayoutDashboard, KanbanSquare, Settings, Users, LogOut, UserCircle2, Sparkles, X, ChevronRight, Terminal, BarChart2, Briefcase, Timer, Palette } from 'lucide-react';
...
export default function Sidebar({ workspace, workspaceId, isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-screen w-[260px] flex-col bg-brand-black px-5 py-7 text-white transition-all duration-350 md:translate-x-0 border-r border-white/10 shadow-2xl ${
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            >
              {emojiMap[workspace?.logo] ? (
                React.createElement(emojiMap[workspace?.logo], { className: "h-4.5 w-4.5 text-brand-black" })
              ) : (
                <span className="text-sm font-black">{workspace?.logo || 'S'}</span>
              )}
            </motion.div>
            <div className="min-w-0">
              <p className="truncate text-sm font-editorial font-bold text-white tracking-wide">{workspace?.name || 'Workspace'}</p>
        ...
        <nav className="flex-1 space-y-1.5 scrollbar-elegant overflow-y-auto">
          {navItems.map((item) => {
            ...
            const path = item.to ? `/workspace/${workspaceId}/${item.to}` : `/workspace/${workspaceId}`;
        ...
        <div className="mt-auto border-t border-white/10 pt-5">
          <div className="flex items-center gap-3 px-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-xs font-editorial font-bold text-white tracking-wide">{user?.name || 'User'}</p>
              <p className="truncate text-[10px] text-white/40 font-medium font-sans-editorial mt-0.5">{user?.email || ''}</p>
            </div>
          </div>
